...
export async function POST(req: NextRequest) {
  ...
  const { course_canvas_id, assignment_canvas_id, model } = body;
  if (!course_canvas_id || !assignment_canvas_id) {
    return NextResponse.json({ error: "course_canvas_id and assignment_canvas_id required" }, { status: 400 });
  }
  ...
  const resp = await fetch(`${agentUrl}/grade/preview`, {
    ...
    body: JSON.stringify({ course_canvas_id, assignment_canvas_id, canvas_token: canvasToken, model }),
  });

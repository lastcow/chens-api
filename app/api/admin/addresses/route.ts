...
export async function GET(req: NextRequest) {
  ...
  const shared  = p.get("shared") ?? "";      // "true" | "false" | ""
  ...
  const [rows, countRows] = await Promise.all([
    ...
    profQuery<{ total: string }>(
      `SELECT COUNT(*) AS total FROM msbiz_addresses a ${where}`,
      values
    ),
  ]);

  const total = parseInt(countRows[0]?.total ?? "0", 10);
  return NextResponse.json({ addresses: rows, total, page, pages: Math.ceil(total / LIMIT) });
}
